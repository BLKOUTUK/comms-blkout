...
| Cleanup + Income | **Done** (Rob: modules with mock data / no purpose → go). comms PR #35 `c6bf8cf`: Agents + Settings retired, Newsletters cut to its working core (and a column-mapping defect fixed), Funding = Pipeline · Opportunities · **Income** (grants per fund, subscriptions + Zeffy receivable, earned with donations separate) from the books measure `finance.income_by_code`. Sidebar 8 items. Verified live 16:08 UTC |
