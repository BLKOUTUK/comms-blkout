...
async function updateQueueStatus(queueId, status, platformPostId, errorMessage) {
    if (!supabase)
        return;
    await supabase
...
export default async function handler(req, res) {
    ...
    try {
        ...
        for (const post of posts) {
            ...
            // Update queue status
            await updateQueueStatus(post.id, result.success ? 'published' : 'failed', result.postId, result.error);
            results.push({
                id: post.id,
            ...
            if (recentPosts && recentPosts.length > 0) {
                ...
                for (const post of recentPosts) {
                    if (post.platform === 'linkedin' && LINKEDIN_ACCESS_TOKEN && post.platform_post_id) {
                        try {
                            const metricsResponse = await fetch(`https://api.linkedin.com/v2/socialActions/${post.platform_post_id}?count=true`, {
                                headers: {
                                    'Authorization': `Bearer ${LINKEDIN_ACCESS_TOKEN}`,
                                    'X-Restli-Protocol-Version': '2.0.0'
                                }
