 ...
 */
export async function handleAggregateIntelligence(_req, res) {
    ...
    try {
        ...
        const activeHubMembers = hubMembers?.filter((m) => m.account_status === 'active').length || 0;
        const verifiedCreators = govMembers?.filter((m) => m.creator_sovereignty_verified === true).length || 0;
        const recentHubJoins = hubMembers?.filter((m) => new Date(m.created_at) > new Date(lastWeek)).length || 0;
        const activeGovMembers = govMembers?.filter((m) => m.membership_status === 'active').length || 0;
        ...
        const intelligenceRecords = [
            ...
            },
            {
                ...
                intelligence_data: { upcoming: upcomingEvents?.length || 0, events: upcomingEvents?.map((e) => e.title) || [] },
                summary: `${upcomingEvents?.length || 0} events in the next 7 days`,
                key_insights: [upcomingEvents?.[0] ? `Next: ${upcomingEvents[0].title}` : 'No upcoming events', 'Plan event promotions'],
            ...
            },
            {
                ...
                intelligence_data: { count: recentArticles?.length || 0, articles: recentArticles?.map((a) => ({ title: a.title, score: a.interest_score })) || [] },
                summary: `${recentArticles?.length || 0} articles published this week`,
                key_insights: [recentArticles?.[0] ? `Top: "${recentArticles[0].title}"` : 'Creating content', 'Weekly publishing active'],
