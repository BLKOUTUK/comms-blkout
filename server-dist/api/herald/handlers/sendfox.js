 ...
 */
export async function handleSendFoxLists(req, res) {
    if (!SENDFOX_API_KEY) {
 ...
 */
export async function handleSendFoxSend(req, res) {
    if (!SENDFOX_API_KEY) {
        return res.status(500).json({ error: 'SendFox API key not configured' });
    }
    ...
    const targetListId = list_id || SENDFOX_LISTS[edition.edition_type] || SENDFOX_LISTS.weekly_engaged;
    try {
        await supabase
            .from('newsletter_editions')
            .update({
