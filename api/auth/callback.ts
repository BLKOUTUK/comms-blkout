...
const PLATFORMS = {
  instagram: {
    ...
    exchangeToken: async (code: string, config: any) => {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (profile.username) successParams.set(`${platform}_username`, profile.username);
    if (profile.name) successParams.set(`${platform}_name`, profile.name);
    if (profile.id) successParams.set(`${platform}_id`, profile.id);
    if (profile.user_id) successParams.set(`${platform}_user_id`, profile.user_id);
    if (profile.open_id) successParams.set(`${platform}_open_id`, profile.open_id);
